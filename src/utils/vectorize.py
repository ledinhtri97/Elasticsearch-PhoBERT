...
class Vectorize:
  ...
  def handle(self):
    ...
    inputs = self.phobert_tokenizer(sentences, padding=True, truncation=True, return_tensors="pt").to(self._device)
    vectors = self._embed_inputs(inputs)
    return vectors

  def _embed_inputs(self, inputs):
    with no_grad():
      phobert_model_output: BaseModelOutputWithPoolingAndCrossAttentions = self.phobert_model(
